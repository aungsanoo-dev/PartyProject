...
namespace F21Party.Controllers
{
    internal class CtrlFrmProfile
    {
        ...
        public void ShowData()
        {
            ...
            frmProfile.txtPassword.Text = dtAccount.Rows[0]["Password"].ToString();
            frmProfile.txtConfirmPassword.Text = dtAccount.Rows[0]["Password"].ToString();
        ...
        public void SaveClick()
        {
            ...
            else if (frmProfile.cboAccessLevel.SelectedValue.ToString() == "0")
            {
                MessageBox.Show("Please Choose AccessLevel");
                frmProfile.cboAccessLevel.Focus();
            }
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        dbaAccountSetting.PASS = Regex.Replace(frmProfile.txtPassword.Text.Trim(), @"\s+", " ");
        ...
        public void AddCombo(ComboBox cboCombo, string spString, string Display, string Value, bool _IsEdit)
        {
            ...
            try
            {
                ...
                {
                    Dr = DTCombo.NewRow();
                    if (frmProfile.cboAccessLevel.DisplayMember == "AccessLevel" && Display == "AccessLevel")
                    {
                        goto Bottom;
                    }
                    //if (Program.UserID == 0)
                    ...
                    if (Display == "AccessLevel" && frmProfile.cboAccessLevel.DisplayMember != "")
                    {
                        //MessageBox.Show(frmProfile.cboAccessLevel.DisplayMember);
                        spAccess2 = string.Format("SP_Select_Access N'{0}',N'{1}',N'{2}'", Convert.ToInt32(frmProfile.cboAccessLevel.DisplayMember), "0", "1");
                    ...
                    if (Program.UserAuthority == 1 && Display == "AccessLevel")
                    {
                        ...
                        //MessageBox.Show(frmProfile.cboAccessLevel.DisplayMember.ToString());
                    ...
                    if (_IsEdit)
                    {
                        ...
                        //int spAccess3 = Convert.ToInt32(frmProfile.cboAccessLevel.DisplayMember);
                        //if(frmProfile.cboAccessLevel.DisplayMember != "AccessLevel")
                        //{


                        if (Display == "AccessLevel" && Convert.ToInt32(dtAccess2.Rows[0]["Authority"]) >= Program.UserAuthority)
                        {
                            //MessageBox.Show(Convert.ToInt32(frmProfile.cboAccessLevel.DisplayMember).ToString());
