    ...
    internal class CtrlFrmCreatePage
    {
        ...
        public void CreateClick()
        {
            ...
            else
            {
                ...
                {
                    dbaPageSetting.PID = Convert.ToInt32(_PageID);
                    dbaPageSetting.PNAME = Regex.Replace(frmCreatePage.txtPageName.Text.Trim(), @"\s+", " ");

                    if (_IsEdit)
                    {
                        dbaPageSetting.PID = Convert.ToInt32(_PageID);
                        dbaPageSetting.ACTION = 1;
                        dbaPageSetting.SaveData();
